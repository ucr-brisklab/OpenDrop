...
#else
SlowSoftWire myWire = SlowSoftWire(P1_pin, P2_pin, true);
#endif
...
void OpenAdapter::begin(void) {
  ...
  // Init Magnet Shield
  if ((_ID & 1) == 1 || magnet_withv40) { // 4.0 and 4.1
...
uint8_t OpenAdapter::get_ID(void) { return _ID; }
...
void myWireBegin() {
  myWire.begin();
  // Assign pins P1_pin and P2_pin to SERCOM functionality
...
void Adafruit_MCP23008::begin(uint8_t addr) {
  ...
  if (!myWire.begin())
    Serial.println("failed to start myWire");
  else
    Serial.println("started myWire");
...
*/
  ...
  if ((ADC->RESULT.reg) > 50) {
    ...
    // Assign P1_pin and P2_pin to SERCOM_ALT mode
    if (!OpenDropV40_downgrade) {
      pinPeripheral(P1_pin, PIO_SERCOM_ALT);
      pinPeripheral(P2_pin, PIO_SERCOM_ALT);
    }
