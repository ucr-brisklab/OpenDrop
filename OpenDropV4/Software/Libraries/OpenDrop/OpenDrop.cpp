...
char version_str[] = "V4.05."; // 4.0 OD with 4.1 magnet module
...
byte cmd_byte0 =
    B00010001; // command byte to write to pot 0, from the MCP42XXX datasheet
...
void TC4_Handler(void) {
  ...
  state = !state;
  // END OF YOUR CODE
  TC4->COUNT16.INTFLAG.bit.MC0 =
      1; // don't change this, it's part of the timer code
}
...
void pinStr(
    uint32_t ulPin,
    unsigned strength) // works like pinMode(), but to set drive strength
{
...
void configureAdc() {
  ...
  ADC->CTRLB.reg = ADC_CTRLB_PRESCALER_DIV512 | // Divide Clock ADC GCLK by 512
                                                // (48MHz/512 = 93.7kHz)
  ...
  ADC->SAMPCTRL.reg =
      ADC_SAMPCTRL_SAMPLEN(63); // Set Sampling Time Length (341.33 us)
...
bool tcIsSyncing() { return TC4->COUNT16.STATUS.reg & TC_STATUS_SYNCBUSY; }
...
void tcConfigure(int sampleRate) {
  ...
  // set TC4 timer counter based off of the system clock and the user defined
...
void spi_transfer(byte working) {
  ...
  for (int i = 1; i <= 8;
       i++) { // setup a loop of 8 iterations, one for each bit

    if (working > 127) { // test the most significant bit

      digitalWrite(
          VSI_pin,
          HIGH); // if it is a 1 (ie. B1XXXXXXX), set the master out pin high
      ...
      digitalWrite(
          VSI_pin,
          LOW); // if it is not 1 (ie. B0XXXXXXX), set the master out pin low
    }

    digitalWrite(
        VSCK_pin,
        HIGH); // set clock high, the pot IC will read the bit into its register

    working = working << 1;

    digitalWrite(VSCK_pin,
                 LOW); // set clock low, the pot IC will stop reading and
                       // prepare for the next iteration (next significant bit
  }
}

void spi_out(int SS, byte cmd_byte,
             byte data_byte) { // SPI tranfer out function begins here

  digitalWrite(SS, LOW); // set slave select low for a certain chip, defined in
                         // the argument in the main loop. selects the chip

  work = cmd_byte; // let the work byte equal the cmd_byte, defined in the
                   // argument in the main loop

  spi_transfer(work); // transfer the work byte, which is equal to the cmd_byte,
                      // out using spi

  work = data_byte; // let the work byte equal the data for the pot

  spi_transfer(
      work); // transfer the work byte, which is equal to the data for the pot

  digitalWrite(SS, HIGH); // set slave select high for a certain chip, defined
                          // in the argument in the main loop. deselcts the chip
}
...
bool free_Fluxel(uint8_t x, uint8_t y, uint8_t dir) {
  ...
  if (check && (x > 0) & (y > 0) && ((dir == 1) || (dir == 4)))
    check = pad_feedback[pgm_read_byte_near(&FluxelID[x - 1][y - 1])] ==
            1; // links oben
  if (check && (x > 0) & (y < (FluxlPad_heigth - 1)) &&
      ((dir == 3) || (dir == 4)))
    check = pad_feedback[pgm_read_byte_near(&FluxelID[x - 1][y + 1])] ==
            1; // links unten
  if (check && (x < (FluxlPad_width - 1)) && (y > 0) &&
      ((dir == 1) || (dir == 2)))
    check = pad_feedback[pgm_read_byte_near(&FluxelID[x + 1][y - 1])] ==
            1; // rechts oben
  if (check && (x < (FluxlPad_width - 1)) && (y < (FluxlPad_heigth - 1)) &&
      ((dir == 2) || (dir == 3)))
    check = pad_feedback[pgm_read_byte_near(&FluxelID[x + 1][y + 1])] ==
            1; // rechts unten
...
OpenDrop::OpenDrop(uint8_t addr) { _addr = addr; }
...

{
  ...
  display.print(" ");
  VOLTAGE_value =
      0.5 * VOLTAGE_value + 0.5 * (analogRead(VSENS_pin) * 3.3 *
                                   (1500000 + 6800 + 5000) / 1024 / 6800);
  if (AC_flag)
    display.print("AC ");
  else
    display.print("DC ");
  ...
  if ((HV_set_ok) &&
      ((abs(VOLTAGE_value - Voltage_set) > Volateg_tollerance))) {
    display.fillRect(4, 33, 121, 12, 1);
    display.setTextColor(0);
    display.setCursor(18, 36);
...
bool OpenDrop::get_Fluxel(int x, int y) { return Fluxls_feedback[x][y]; }
...

{
  ...
  {
    digitalWrite(DI_pin, Fluxls[pgm_read_byte_near(pad_lookup_x + i)]
                               [pgm_read_byte_near(pad_lookup_y + i)]);
    delayMicroseconds(chip_delay);
  ...
  for (int i = 0; i < 64; i++) {
    digitalWrite(DI_pin, Fluxls[15 - pgm_read_byte_near(pad_lookup_x + i)]
                               [7 - pgm_read_byte_near(pad_lookup_y + i)]);
    delayMicroseconds(chip_delay);
...
void OpenDrop::read_Fluxels(void) {
  ...
  if (HV_enable) {
    ...
    for (int i = 0; i < 128; i++) // 128
    {
      ...
      if (i < 64)
        Fluxls_feedback[15 - pgm_read_byte_near(pad_lookup_x + 63 - i)]
                       [7 - pgm_read_byte_near(pad_lookup_y + 63 - i)] =
                           reading;
      else
        Fluxls_feedback[pgm_read_byte_near(pad_lookup_x + 127 - i)]
                       [pgm_read_byte_near(pad_lookup_y + 127 - i)] = reading;
...
void OpenDrop::begin(char code_str[]) {
  ...
  //     pinStr(P1_pin,1);       //Set Pin output current to 7mA (bug in the
  //     Arduino library?)

  //     pinStr(P2_pin,1);       //Set Pin output current to 7mA (bug in the
  //     Arduino library?)

  pinMode(BL_pin, OUTPUT);         // BL pin
  digitalWrite(BL_pin, HV_enable); // set BL
  pinStr(BL_pin,
         1); // Set Pin output current to 7mA (bug in the Arduino library?)
  ...
  digitalWrite(VCS_pin, HIGH); // hold slave select 1 pin high, so that chip is
                               // not selected to begin with
  ...
  digitalWrite(AC_pin, HIGH); // Re-Init set AC  (gets messed up in
                              // getAdapterID)

  OpenDropID =
      (board_ID << 4) | Adapter.get_ID(); // OpenDrop ID = board_ID higher 4
                                          // bits, Adapter ID lower 4 bits
  ...
  for (int i = 0; i < this->drop_count; i++) {
    ...
    if (feedback_control) {
      ...
      if ((drops[i].position_x() != drops[i].next_x()) |
        ...
        Serial.println("run");
        if ((pad_feedback[pgm_read_byte_near(
                 &FluxelID[drops[i].position_x()][drops[i].position_y()])] ==
             0) &
            (pad_feedback[pgm_read_byte_near(
...
bool OpenDrop::run(void) {
  ...
  for (int i = 0; i < this->drop_count; i++) {
    ...
    _runing = ((drops[i].position_x() != drops[i].goal_x()) |
               (drops[i].position_y() != drops[i].goal_y()));
    ...
    if (_runing & !drops[i].is_moving()) {
      ...
      if (right)
        d1 = pow((drops[i].position_x() + 1) - (drops[i].goal_x()), 2) +
             pow((drops[i].position_y()) - (drops[i].goal_y()), 2);

      if (left) {
        d2 = pow((drops[i].position_x() - 1) - (drops[i].goal_x()), 2) +
             pow((drops[i].position_y()) - (drops[i].goal_y()), 2);
        if (d2 < d1) {
          right = false;
          d1 = d2;
      ...
      */
      ...
      if (up) {
        d2 = pow((drops[i].position_x()) - (drops[i].goal_x()), 2) +
             pow((drops[i].position_y() - 1) - (drops[i].goal_y()), 2);
        if (d2 < d1) {
          left = false;
          right = false;
          d1 = d2;
...
void OpenDrop::set_voltage(uint16_t voltage, bool AC_on, uint16_t frequence) {
  ...
  VOLTAGE_set_value =
      255 * ((1.5 * 1500000) / (voltage - 1.5) - 6800) /
      10000; // Calculate set value (max 255) for 10kOhm digial potentiometer
             // from reference voltage 1.5V, voltage devider R1.5MOhm / 6.8kOhm,
#else
  VOLTAGE_set_value =
      255 * ((1.5 * 1500000) / (voltage - 1.5) - 6800) /
      50000; // Calculate set value (max 255) for 50kOhm digial potentiometer
             // from reference voltage 1.5V, voltage divider R1.5MOhm / 6.8kOhm,
#endif

  spi_out(
      VCS_pin, cmd_byte0,
      VOLTAGE_set_value); // Set Voltage Level / send out data to chip 1, pot 0
  Voltage_set = voltage;
  AC_flag = AC_on;
}
...
uint8_t OpenDrop::get_ID() { return OpenDropID; }
...
void OpenDrop::show_joy(boolean val) { _show_joy = val; }

void OpenDrop::show_feedback(boolean val) { feedback = val; }
...
void OpenDrop::set_Temp_1(uint8_t temperature) {
  Adapter.set_Temp_1(temperature);
};
void OpenDrop::set_Temp_2(uint8_t temperature) {
  Adapter.set_Temp_2(temperature);
};
void OpenDrop::set_Temp_3(uint8_t temperature) {
  Adapter.set_Temp_3(temperature);
};
...
int Drop::position_x(void) { return _pos_x; }

int Drop::position_y(void) { return _pos_y; }

int Drop::next_x(void) { return _next_x; }

int Drop::next_y(void) { return _next_y; }

int Drop::goal_x(void) { return _goal_x; }

int Drop::goal_y(void) { return _goal_y; }

int Drop::num(void) { return _dropnum; }
...
bool Drop::is_moving(void) { return _moving; }
...
void Menu(OpenDrop &theOpenDrop) {
  ...
  while (!confirm) {
    ...
    if ((JOY_value > 725) && (JOY_value < 895) && (menu_position > 1) &&
        (nav_release)) {
      menu_position--;
      nav_release = false;
      set_confirm = false;
    }
    if ((JOY_value > 256) && (JOY_value < 597) && (menu_position < 6) &&
        (nav_release)) {
      menu_position++;
      nav_release = false;
    }
